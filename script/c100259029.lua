...
function s.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.GetMatchingGroup(s.cfilter2,tp,LOCATION_MZONE,0,1,nil)
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil)
	local tc=g:GetFirst()
	local op=0
		if #tg>0 and tc:IsCanBeSpecialSummoned(e,0,tp,false,false) then op=Duel.SelectOption(tp,aux.Stringid(id,2),aux.Stringid(id,3))
		elseif #tg>0 then op=0
		else op=1 end
		if op==0 then
			local oc=tg:Select(tp,1,1,nil):GetFirst()
			Duel.Overlay(oc,tc)
		else
