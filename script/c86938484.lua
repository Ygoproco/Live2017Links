...
function c86938484.cfilter(c)
	return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsSetCard(0x9d) and c:IsAbleToGraveAsCost()
end
function c86938484.mzfilter(c)
	return c:IsLocation(LOCATION_MZONE) and c:GetSequence()<5
end
function c86938484.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local sg=Duel.GetMatchingGroup(c86938484.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,nil)
	if chk==0 then return e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false)
		and sg:GetCount()>=1 and (ft>0 or sg:IsExists(c86938484.mzfilter,1,nil)) end
...
function c86938484.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if ft<=0 then
		...
		g=sg:FilterSelect(tp,c86938484.mzfilter,1,1,nil)		
	else
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
