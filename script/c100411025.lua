...
function s.spcon(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler(e,tp,eg,ep,ev,re,r,rp)
	return c:IsLocation(LOCATION_GRAVE) and r==REASON_SYNCHRO
end
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x223) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.thfilter(c)
	return c:IsCode(CODE_FROZEN_ROAR,CODE_BLOOMING_ROSE) and c:IsAbleToHand()
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,e,tp)
	if #g>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>1 and e:GetLabel()==1 
