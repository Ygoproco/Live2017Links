...
function c101004087.spfilter(c)
...
function c101004087.spcon(e,c)
	...
	if Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)<=0 
		or Duel.IsExistingMatchingCard(Card.IsCode,c:GetControler(),LOCATION_MZONE,0,1,nil,101004087) then return false end
	return Duel.IsExistingMatchingCard(c101004087.spfilter,c:GetControler(),LOCATION_MZONE,0,1,nil)
end
...
function c101004087.rdop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFacedown() or not c:IsRelateToEffect(e) or c:IsImmuneToEffect(e) or c:GetLevel()<4 then return end
	local e1=Effect.CreateEffect(c)
