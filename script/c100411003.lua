...
function c100411003.initial_effect(c)
	...
	--special summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100411003,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e1:SetCode(EVENT_DESTROYED)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCountLimit(1,100411003)
	e1:SetCondition(c100411003.spcon)
	e1:SetTarget(c100411003.sptg)
	e1:SetOperation(c100411003.spop)
	c:RegisterEffect(e1)
	--spsummon
	...
	e2:SetCountLimit(1,100411103)
	e2:SetCondition(c100411003.spcon2)
	e2:SetTarget(c100411003.sptg2)
	e2:SetOperation(c100411003.spop2)
	c:RegisterEffect(e2)
end
...
function c100411003.spfilter(c,e,tp,code)
	return c:IsCode(0x64) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:GetOriginalCode()~=code
end
...
function c100411003.filter(c,e,tp)
	return c:IsCode(0x64) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
