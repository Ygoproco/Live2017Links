...
function c70026064.filter1(c,e,tp)
	...
	return pg:GetCount()<=1 and c:IsFaceup() and c:IsSetCard(0x88)
		and Duel.IsExistingMatchingCard(c70026064.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,c:GetCode(),pg)
		and Duel.GetLocationCountFromEx(tp,tp,c)>0
end
function c70026064.filter2(c,e,tp,mc,code,pg)
...
function c70026064.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c70026064.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetCode(),pg)
	local sc=g:GetFirst()
	if sc then
