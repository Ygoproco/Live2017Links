...
function c67225377.spcfilter(c)
	return c:IsSetCard(0x119) and c:IsAbleToGraveAsCost()
end
function c67225377.spcost2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c67225377.spcfilter,tp,LOCATION_HAND,0,1,e:GetHandler()) end
	Duel.DiscardHand(tp,c67225377.spcfilter,1,1,REASON_COST+REASON_DISCARD,e:GetHandler())
end
function c67225377.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
