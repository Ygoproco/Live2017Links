...
function c98446407.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>-1 and c:IsReleasable() and Duel.CheckReleaseGroupCost(tp,nil,1,false,nil,c)
		and (ft>0 or Duel.CheckReleaseGroupCost(tp,c98446407.mzfilter,1,false,nil,c,tp)) end
	local rg=nil
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	if ft>0 then
		rg=Duel.SelectReleaseGroupCost(tp,nil,1,1,false,nil,c)
	else
		rg=Duel.SelectReleaseGroupCost(tp,c98446407.mzfilter,1,1,false,nil,c,tp)
	end
	rg:AddCard(c)
