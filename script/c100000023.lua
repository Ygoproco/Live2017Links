...
function c100000023.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	c100000023.min_material_count=2
	c100000023.max_material_count=2
...
function c100000023.ffilter(c,fc,tp)
	if not c:IsCanBeFusionMaterial(fc) then return false end
	return c100000023.filter1(c,tp) or c100000023.filter2(c,tp)
end
function c100000023.FCheckGoal(tp,sg,fc)
	local g=Group.CreateGroup()
	return sg:IsExists(c100000023.FCheck,1,nil,tp,sg,g,c100000023.filter1,c100000023.filter2) and Duel.GetLocationCountFromEx(tp,tp,sg,fc)>0
		and (not aux.FCheckAdditional or aux.FCheckAdditional(tp,sg,fc))
end
function c100000023.FCheck(c,tp,mg,sg,fun1,fun2)
	if fun2 then
		sg:AddCard(c)
		mg:RemoveCard(c)
		local res=fun1(c,tp) and mg:IsExists(c100000023.FCheck,1,sg,tp,mg,sg,fun2)
		sg:RemoveCard(c)
		mg:AddCard(c)
		return res
	else
...
function c100000023.filterchk(c,tp,mg,sg,fc)
	sg:AddCard(c)
	mg:RemoveCard(c)
	local res
	...
	if sg:GetCount()<2 then
		res=mg:IsExists(c100000023.filterchk,1,tp,mg,sg,fc)
	else
		res=c100000023.FCheckGoal(tp,sg,fc)
	end
	sg:RemoveCard(c)
	mg:AddCard(c)
	mg:Merge(rg)
...
function c100000023.fcon(e,g,gc,chkfnf)
	...
	local mg=g:Filter(c100000023.ffilter,c,c,tp)
	if gc then
		if not mg:IsContains(gc) then return false end
		return c100000023.filterchk(gc,tp,mg,Group.CreateGroup(),fc)
	end
	local sg=Group.CreateGroup()
	return mg:IsExists(c100000023.filterchk,1,nil,tp,mg,sg,c)
end
function c100000023.fop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	local tp=e:GetHandlerPlayer()
	local mg=eg:Filter(c100000023.ffilter,c,c,tp)
	local p=tp
	local sfhchk=false
