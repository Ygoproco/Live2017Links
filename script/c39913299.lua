...
function c39913299.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	c39913299.announce_filter={TYPE_EXTRA,OPCODE_ISTYPE,OPCODE_NOT}
...
function c39913299.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsCode(ac) and tc:IsAbleToHand() then
		...
		if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(39913299,0)) then
			Duel.BreakEffect()
			...
			local op=0
			if b1 and b2 then op=Duel.SelectOption(tp,aux.Stringid(39913299,1),aux.Stringid(39913299,2))
			elseif b1 then op=Duel.SelectOption(tp,aux.Stringid(39913299,1))
			else op=Duel.SelectOption(tp,aux.Stringid(39913299,2))+1 end
			if op==0 then
