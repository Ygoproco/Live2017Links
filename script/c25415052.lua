	...
	if chk==0 then
		if e:IsHasType(EFFECT_TYPE_TRIGGER_F) then
			...
			return Duel.GetLocationCount(tp,LOCATION_MZONE)>1
				...
				and Duel.IsPlayerCanSpecialSummonMonster(tp,25415053,0,TYPES_TOKEN,1500,1500,4,RACE_WINDBEAST,ATTRIBUTE_WIND)
		end
	end
...
function c25415052.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SendtoHand(c,nil,REASON_EFFECT)~=0
		...
		and not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,25415053,0,TYPES_TOKEN,1500,1500,4,RACE_WINDBEAST,ATTRIBUTE_WIND) then
		for i=1,2 do
