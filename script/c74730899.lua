...
function c74730899.retcfilter(c)
	return c:IsSetCard(0x28)
end
function c74730899.retfilter(c,e)
	return c:IsAbleToHand() and c:IsCanBeEffectTarget(e)
end
function c74730899.retcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c74730899.retfilter,tp,0,LOCATION_ONFIELD,nil,e)
	if chk==0 then return Duel.CheckReleaseGroupCost(tp,c74730899.retcfilter,1,false,aux.ReleaseCheckTarget,e:GetHandler(),dg) end
	local g=Duel.SelectReleaseGroupCost(tp,c74730899.retcfilter,1,1,false,aux.ReleaseCheckTarget,e:GetHandler(),dg)
