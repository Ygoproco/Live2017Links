...
function c100000285.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetAttacker():IsControler(1-tp) and Duel.IsExistingMatchingCard(c100000285.cfilter,tp,LOCATION_MZONE,0,1,nil)
end
function c100000285.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return tg:IsOnField() and tg:IsCanBeEffectTarget(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
		and Duel.IsExistingMatchingCard(c100000285.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
...
function c100000285.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tg or not tg:IsRelateToEffect(e) or tg:IsStatus(STATUS_ATTACK_CANCELED)
		or not Duel.NegateAttack() or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
