...
function c94620082.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(94620082,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DESTROY)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_GRAVE)
	e2:SetCountLimit(1,94620082)
	e2:SetCondition(c94620082.spcond)
...
function c94620082.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:IsExists(c94620082.thfilter,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(94620082,2)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local sg=g:FilterSelect(tp,c94620082.thfilter,1,1,nil)
...
function c94620082.costfilter(c)
	return c:IsSetCard(0x119) and c:IsAbleToGraveAsCost()
end
function c94620082.spcond(e,tp,eg,ep,ev,re,r,rp)
...
function c94620082.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(c94620082.costfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c94620082.costfilter,tp,LOCATION_HAND,0,1,1,c)
...
function c94620082.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(94620082,3)) then
