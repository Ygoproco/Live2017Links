...
function c28770951.tdfilter(c,e)
	return (c:IsSetCard(0x55) or c:IsSetCard(0x7b)) and c:IsAbleToDeck() and c:IsCanBeEffectTarget(e)
end
function c28770951.rescon(sg,e,tp,mg)
	return sg:GetClassCount(Card.GetCode)==#sg
end
function c28770951.drtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	local tg=Duel.GetMatchingGroup(c28770951.tdfilter,tp,LOCATION_GRAVE,0,nil,e)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,2)
		and aux.SelectUnselectGroup(tg,e,tp,5,5,c28770951.rescon,0) end
	local g=aux.SelectUnselectGroup(tg,e,tp,5,5,c28770951.rescon,1,tp,HINTMSG_TODECK)
