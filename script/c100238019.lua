...
function s.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsPlayerCanSpecialSummonMonster(tp,id+100,0,0x4011,100,100,1,RACE_PYRO,ATTRIBUTE_FIRE) end
    Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0)
    Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
end
function s.tkop1(e,tp,eg,ep,ev,re,r,rp)
    if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsPlayerCanSpecialSummonMonster(tp,id+100,0,0x4011,100,100,1,RACE_PYRO,ATTRIBUTE_FIRE) then
...
function s.tkop2(e,tp,eg,ep,ev,re,r,rp)
    ...
    ft=math.min(ct,ft)
    if ft<1 or not Duel.IsPlayerCanSpecialSummonMonster(tp,id+100,0,0x4011,100,100,1,RACE_PYRO,ATTRIBUTE_FIRE) then return end
