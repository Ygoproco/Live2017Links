...
function c101005052.atkval(e,c)
	local val=Duel.GetMatchingGroupCount(c101005052.atkfilter,e:GetHandlerPlayer(),LOCATION_MZONE,LOCATION_MZONE,nil)*800
	e:SetLabel(val)
	return val
end
function c101005052.atkfilter(c,tc)
	return c:IsType(TYPE_LINK) and c:IsSummonType(SUMMON_TYPE_LINK) and c:GetFlagEffect(101005052)
end
...
function c101005052.matop(e,tp,eg,ep,ev,re,r,rp)
	eg:Filter(c101005052.matfilter,nil,e:GetHandler()):GetFirst():GetReasonCard():RegisterFlagEffect(101005052,RESET_EVENT+0x1ee0000,0,1)
end
