...
function c65357623.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(65357623,1))
...
function c65357623.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() then
		local atk=1000
		if Duel.SelectYesNo(tp,aux.Stringid(65357623,0)) then atk=atk*-1 end 
...
function c65357623.setfilter(c)
...
function c65357623.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsSSetable()
		and Duel.IsExistingMatchingCard(c65357623.setfilter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,e:GetHandler(),1,0,0)
end
function c65357623.setop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,c65357623.setfilter,tp,LOCATION_REMOVED,0,1,1,nil)
