...
function c3560069.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(3560069,1))
	e2:SetCategory(CATEGORY_DRAW)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e2:SetCode(EVENT_BE_MATERIAL)
	e2:SetCountLimit(1,3560070)
	e2:SetCondition(c3560069.condition)
	e2:SetTarget(c3560069.target)
	e2:SetOperation(c3560069.operation)
	c:RegisterEffect(e2)
end
...
function c3560069.condition(e,tp,eg,ep,ev,re,r,rp)
	return r==REASON_LINK and e:GetHandler():GetFlagEffect(511600046)>0
end
function c3560069.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c3560069.operation(e,tp,eg,ep,ev,re,r,rp)
