...
function c72529749.cfilter(c,zone,tp)
	local seq=c:GetSequence()
	if c:IsControler(1-tp) then seq=seq+16 end
	return bit.extract(zone,seq)~=0
end
function c72529749.rmcon(e,tp,eg,ep,ev,re,r,rp)
	return not eg:IsContains(e:GetHandler()) and eg:IsExists(c72529749.cfilter,1,nil,e:GetHandler():GetLinkedZone(),tp)
end
function c72529749.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local g=eg:Filter(c72529749.cfilter,nil,e:GetHandler():GetLinkedZone(),tp)
...
function c72529749.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=eg:Filter(c72529749.cfilter,nil,c:GetLinkedZone(),tp)
	local g2=Duel.GetMatchingGroup(Card.IsType,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil,TYPE_SPELL+TYPE_TRAP)
	g:Merge(g2)
