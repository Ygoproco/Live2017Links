...
function s.atkupcond(e,tp,eg,ep,ev,re,r,rp)
    ...
    return (re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_SPELL+TYPE_TRAP) and re:GetHandler():IsSetCard(0x226) and re:GetHandler()~=e:GetHandler())
	or re:GetHandler()==ec
end
function s.atkupop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ec=c:GetEquipTarget()
		...
		--previously used to increase the attack
...
function s.rfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsSetCard(0x226) and c:IsAbleToRemoveAsCost() and not c:IsCode(id)
end
