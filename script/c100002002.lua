...
function c100002002.filter3(c,e,tp,m,f,chainm)
	return c:IsType(TYPE_FUSION) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) 
		and m:IsExists(c100002002.matfilter1,1,nil,m,c,tp,chainm)
end
function c100002002.matfilter1(c,m,fc,tp,chainm)
	return (c:IsOnField() or chainm) and m:IsExists(c100002002.matfilter2,1,c,fc,tp,c,chainm)
end
function c100002002.matfilter2(c,fc,tp,mc,chainm)
...
function c100002002.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		mg1:Merge(mg2)
		local res=Duel.IsExistingMatchingCard(c100002002.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,false)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c100002002.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,true)
			end
		end
		return res
	end
...
function c100002002.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	mg1:Merge(mg)
	local sg1=Duel.GetMatchingGroup(c100002002.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,false)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c100002002.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,true)
	end
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			...
			local mat1=mg1:FilterSelect(tp,c100002002.matfilter1,1,1,nil,mg1,tc,tp,false)
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local mat2=mg1:FilterSelect(tp,c100002002.matfilter2,1,1,mat1:GetFirst(),tc,tp,mat1:GetFirst(),false)
			mat1:Merge(mat2)
			tc:SetMaterial(mat1)
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local mat1=mg2:FilterSelect(tp,c100002002.matfilter1,1,1,nil,mg2,tc,tp,true)
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local mat2=mg2:FilterSelect(tp,c100002002.matfilter2,1,1,mat1:GetFirst(),tc,tp,mat1:GetFirst(),true)
			mat1:Merge(mat2)
