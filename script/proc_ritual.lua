...
--Ritual Summon
function Auxiliary.CreateRitualProc(c,_type,filter,lv,desc,extrafil,extraop,forcedgroup,stage2,location)
	...
	e1:SetTarget(Auxiliary.RPTarget(filter,_type,lv,extrafil,extraop,forcedgroup,stage2,location))
	e1:SetOperation(Auxiliary.RPOperation(filter,_type,lv,extrafil,extraop,forcedgroup,stage2,location))
	return e1
end
function Auxiliary.AddRitualProc(c,_type,filter,lv,desc,extrafil,extraop,forcedgroup,stage2,location)
	local e1=aux.CreateRitualProc(c,_type,filter,lv,desc,extrafil,extraop,forcedgroup,stage2,location)
	c:RegisterEffect(e1)
	return e1
end
function Auxiliary.RPFilter(c,filter,_type,e,tp,m,m2,ft,lv)
	...
	if c.ritual_custom_condition then
		return c:ritual_custom_condition(mg,ft,_type)
	end
...
function Auxiliary.RPTarget(filter,_type,lv,extrafil,extraop,forcedgroup,stage2,location)
	return	function(e,tp,eg,ep,ev,re,r,rp,chk)
				...
				if chk==0 then
					...
					return ft>-1 and Duel.IsExistingMatchingCard(Auxiliary.RPFilter,tp,location,0,1,nil,filter,_type,e,tp,mg,mg2,ft,lv)
				end
				Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,location)
			end
end
function Auxiliary.RPOperation(filter,_type,lv,extrafil,extraop,forcedgroup,stage2,location)
	return	function(e,tp,eg,ep,ev,re,r,rp)
				...
				local tg=Duel.SelectMatchingCard(tp,Auxiliary.RPFilter,tp,location,0,1,1,nil,filter,_type,e,tp,mg,mg2,ft,lv)
				...
				if tc then
					...
					if tc.ritual_custom_operation then
						tc.ritual_custom_operation(mg,_type)
						mat=tc:GetMaterial()
					else
