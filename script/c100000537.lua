...
function c100000537.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(1-tp) and c100000537.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c100000537.filter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c100000537.filter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,1,nil)
