...
function c15520842.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local ged=Duel.IsExistingMatchingCard(c15520842.geffilter,tp,LOCATION_MZONE,0,1,nil)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c15520842.filter(chkc,tp,ged) end
	if chk==0 then return Duel.IsExistingTarget(c15520842.filter,tp,0,LOCATION_MZONE,1,nil,tp,ged) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
	local g=Duel.SelectTarget(tp,c15520842.filter,tp,0,LOCATION_MZONE,1,1,nil,tp,ged)
