...
function c101006034.initial_effect(c)
    c:EnableReviveLimit()
    aux.AddFusionProcMixRep(c,true,true,aux.FilterBoolFunctionEx(Card.IsType,TYPE_LINK),1,99,101006001)
    --mill and atk
    local e1=Effect.CreateEffect(c)
    e1:SetCategory(CATEGORY_TOGRAVE+CATEGORY_ATKCHANGE+CATEGORY_DECKDES)
    e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
    e1:SetCode(EVENT_SPSUMMON_SUCCESS)
    e1:SetCondition(c101006034.gycon)
    e1:SetTarget(c101006034.gytg)
    e1:SetOperation(c101006034.gyop)
    c:RegisterEffect(e1)
    --cannot be target
    ...
    e2:SetCondition(c101006034.tgcon)
    e2:SetTarget(c101006034.tgtg)
    ...
    e3:SetCondition(c101006034.tgcon)
    e3:SetValue(c101006034.tgtg)
    c:RegisterEffect(e3)
    --spsummon
...
function c101006034.gycon(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION)
end
function c101006034.gytg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101006034.gyop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    local ct=c:GetMaterial():Filter(Card.IsType,nil,TYPE_LINK):GetSum(Card.GetLink)
    if Duel.DiscardDeck(tp,ct,REASON_EFFECT)>0 and c:IsRelateToEffect(e) and c:IsFaceup() then
		...
		e2:SetTargetRange(LOCATION_MZONE,0)
		e2:SetTarget(c101006034.ftarget)
