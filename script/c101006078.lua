...
function c101006078.setfilter(c,cd)
...
function c101006078.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetOperation(c101006078.aclimit1)
	Duel.RegisterEffect(e1,tp)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_FIELD)
	e2:SetCode(EVENT_CHAIN_NEGATED)
	e2:SetReset(RESET_PHASE+PHASE_END)
	e2:SetOperation(c101006078.aclimit2)
	...
	if rc and Duel.Remove(rc,POS_FACEUP,REASON_EFFECT)>0 then
		...
		local g=Duel.SelectMatchingCard(tp,c101006078.setfilter,tp,LOCATION_DECK,0,1,1,nil,rc:GetCode())
		if #g>0 then
			Duel.SSet(tp,g)
...
function c101006078.econ(e)
	return Duel.GetFlagEffect(tp,101006078)~=0
end
function c101006078.elimit(e,te,tp)
