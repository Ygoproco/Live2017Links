...
function c76075810.descfilter(c)
	return c:IsRace(RACE_WARRIOR)
end
function c76075810.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c76075810.filter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,e:GetHandler():GetAttack(),e)
	if chk==0 then return Duel.CheckReleaseGroupCost(tp,c76075810.descfilter,1,false,aux.ReleaseCheckTarget,nil,dg) end
	local g=Duel.SelectReleaseGroupCost(tp,c76075810.descfilter,1,1,false,aux.ReleaseCheckTarget,nil,dg)
	Duel.Release(g,REASON_EFFECT)
end
function c76075810.filter(c,atk,e)
