...
function c33252803.filter1(c,e,tp)
	...
	return pg:GetCount()<=1 and c:IsFaceup() and c:IsSetCard(0x48) and not c:IsSetCard(0x1048) and m and (c:GetRank()>0 or c:IsStatus(STATUS_NO_LEVEL)) 
		and Duel.IsExistingMatchingCard(c33252803.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,c:GetRank()+1,m.xyz_number,pg)
		and Duel.GetLocationCountFromEx(tp,tp,c)>0
end
function c33252803.filter2(c,e,tp,mc,rk,no,pg)
...
function c33252803.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c33252803.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank()+1,m.xyz_number,pg)
	local sc=g:GetFirst()
	if sc then
