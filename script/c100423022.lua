...
function s.initial_effect(c)
	c:EnableReviveLimit()
	aux.AddFusionProcMix(c,true,true,aux.FilterBoolFunction(Card.IsFusionSetCard,0x235),aux.FilterBoolFunctionEx(Card.IsRace,RACE_DRAGON))
	--spsumon
...
function s.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x235) and c:IsLevelAbove(1)
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp,c)
end
function s.spfilter(c,e,tp,tc)
	return c:IsSetCard(0x235) and c:IsLevelAbove(1)
