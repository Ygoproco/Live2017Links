...
function c63741331.filter(c,e)
	return c:IsFaceup() and c:IsCanBeEffectTarget(e)
end
function c63741331.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c63741331.filter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,e)
	if chk==0 then return Duel.CheckReleaseGroupCost(tp,c63741331.cfilter,1,false,aux.ReleaseCheckTarget,nil,dg) end
	local g=Duel.SelectReleaseGroupCost(tp,c63741331.cfilter,1,1,false,aux.ReleaseCheckTarget,nil,dg)
