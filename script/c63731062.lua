...
function c63731062.initial_effect(c)
	...
	e1:SetCondition(c63731062.spcon)
	e1:SetTarget(c63731062.sptg)
	e1:SetOperation(c63731062.spop)
	c:RegisterEffect(e1)
	--special summon self
	...
	e2:SetCondition(c63731062.condition)
	e2:SetCost(c63731062.cost)
	e2:SetTarget(c63731062.target)
	e2:SetOperation(c63731062.operation)
	c:RegisterEffect(e2)
end
function c63731062.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO)
end
function c63731062.spfilter(c,e,tp)
	return c:IsRace(RACE_FISH) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c63731062.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c63731062.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c63731062.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c63731062.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c63731062.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,e:GetHandler(),tp)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c63731062.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c63731062.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
