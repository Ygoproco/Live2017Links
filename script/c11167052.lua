...
function c11167052.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local b1=c11167052.spcost(e,tp,eg,ep,ev,re,r,rp,0) and c11167052.sptg(e,tp,eg,ep,ev,re,r,rp,0)
	local b2=c11167052.thcost(e,tp,eg,ep,ev,re,r,rp,0) and c11167052.thtg(e,tp,eg,ep,ev,re,r,rp,0)
	if (b1 or b2) and Duel.SelectYesNo(tp,94) then
		local op=0
		if b1 and b2 then op=Duel.SelectOption(tp,aux.Stringid(11167052,0),aux.Stringid(11167052,1))
		elseif b1 then op=Duel.SelectOption(tp,aux.Stringid(11167052,0))
		else op=Duel.SelectOption(tp,aux.Stringid(11167052,1))+1 end
		if op==0 then
			e:SetCategory(CATEGORY_SPECIAL_SUMMON)
			e:SetOperation(c11167052.spop)
			c11167052.spcost(e,tp,eg,ep,ev,re,r,rp,1)
			c11167052.sptg(e,tp,eg,ep,ev,re,r,rp,1)
		else
			e:SetCategory(CATEGORY_HANDES+CATEGORY_TOHAND)
			e:SetOperation(c11167052.thop)
			c11167052.thcost(e,tp,eg,ep,ev,re,r,rp,1)
			c11167052.thtg(e,tp,eg,ep,ev,re,r,rp,1)
		end
	else
		e:SetCategory(0)
		e:SetOperation(nil)
...
function c11167052.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.CheckReleaseGroupCost(tp,c11167052.filter1,1,false,nil,nil,e,tp)
	end
	local rg=Duel.SelectReleaseGroupCost(tp,c11167052.filter1,1,1,false,nil,nil,e,tp)
