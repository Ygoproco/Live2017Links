...
function c33846209.spcheck(sg,tp,exg,dg)
	local a=0
...
function c33846209.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		if e:GetLabel()==1 then
			e:SetLabel(0)
			return Duel.CheckReleaseGroupCost(tp,c33846209.cfilter,1,false,c33846209.spcheck,nil,dg)
		else
			return Duel.IsExistingTarget(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,e:GetHandler())
		end
	end
	if e:GetLabel()==1 then
		local sg=Duel.SelectReleaseGroupCost(tp,c33846209.cfilter,1,1,false,c33846209.spcheck,nil,dg)
