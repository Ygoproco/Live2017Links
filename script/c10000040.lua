...
function c10000040.rescon(sg,e,tp,mg)
	-- Debug.Message(sg:IsExists(c10000040.chk,1,nil,sg,Group.CreateGroup(),10000000,10000010,10000020))
...
function c10000040.chk(c,sg,g,code,...)
	local code1,code2=c:GetOriginalCodeRule()
	Debug.Message(tostring(code~=code1 and code~=code2))
...
function c10000040.spcon(e,c)
	...
	g:Merge(g2)
	g:Merge(g3)
	-- Debug.Message(tostring(g:GetCount()>2).." | "..tostring(g1:GetCount()>0).." | "..tostring(g2:GetCount()>0).." | "..tostring(g3:GetCount()>0).." | "..tostring(aux.SelectUnselectGroup(g,e,tp,3,3,c10000040.rescon,0)))
