...
c100200152.listed_names={CARD_CYBER_DRAGON }
function c100200152.cfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsDiscardable()
end
function c100200152.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c100200152.spfilter(c,e,tp)
...
function c100200152.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c100200152.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c100200152.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c100200152.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
