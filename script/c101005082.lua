...
function c101005082.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return not c:IsPublic() and c:GetFlagEffect(101005082)==0 end
	c:RegisterFlagEffect(101005082,RESET_CHAIN,0,1)
end
...
function c101005082.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101005082.spfilter,tp,LOCATION_HAND,0,1,nil,e,tp) and Duel.IsPlayerCanDraw(tp) end
...
function c101005082.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc:GetFirst():IsCode(101005082) then
		Duel.BreakEffect()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sc=Duel.SelectMatchingCard(tp,c101005082.spfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
