...
function c101006050.exmfilter(c)
	return c:GetSequence()>=5
end
function c101006050.filter(c,e)
...
function c101006050.spfilter(c,e,tp,m,f,chkf)
...
function c101006050.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local res=Duel.IsExistingMatchingCard(c101006050.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c101006050.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c101006050.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetFusionMaterial(tp):Filter(c101006050.filter,nil,e)
	local exmat=false
	if not Duel.IsExistingMatchingCard(c101006050.exmfilter,tp,LOCATION_MZONE,0,1,nil) then
	...
	local sg1=Duel.GetMatchingGroup(c101006050.spfilter,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
	Auxiliary.FCheckAdditional=nil
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
