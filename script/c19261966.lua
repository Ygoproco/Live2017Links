...
function c19261966.ffilter2(c,fc,sumtype,tp)
	return (c:IsHasEffect(511002961) or c:IsAttribute(ATTRIBUTE_WATER,fc,sumtype,tp) or c:IsHasEffect(4904633)) and not c:IsHasEffect(6205579)
end
function c19261966.exfilter(c,g,fc,sumtype,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial(fc) and not g:IsContains(c) and (c19261966.ffilter1(c) or c19261966.ffilter2(c,fc,sumtype,tp))
end
function c19261966.ffilter(c,fc,sumtype,tp)
	return c:IsCanBeFusionMaterial(fc) and (c19261966.ffilter1(c) or c19261966.ffilter2(c,fc,sumtype,tp))
end
function c19261966.FCheckMix(c,mg,sg,fc,sub,sub2,fun1,fun2,...)
	if fun2 then
		sg:AddCard(c)
		local res=false
		if fun1(c,fc,SUMMON_TYPE_FUSION,tp) then
			res=mg:IsExists(c19261966.FCheckMix,1,sg,mg,sg,fc,sub,sub2,fun2,...)
		end
		sg:RemoveCard(c)
		return res
	else
...
function c19261966.FCheckMixGoal(tp,sg,fc,sub,sub2,chkf,...)
	local g=Group.CreateGroup()
	return sg:IsExists(c19261966.FCheckMix,1,nil,sg,g,fc,sub,sub2,...) and (chkf==PLAYER_NONE or Duel.GetLocationCountFromEx(tp,tp,sg,fc)>0)
		and (not Auxiliary.FCheckAdditional or Auxiliary.FCheckAdditional(tp,sg,fc))
end
function c19261966.filterchk(c,tp,mg,sg,exg,mustg,fc,chkf)
	local res
	...
	if sg:GetCount()<2 then
		...
		res=mg:IsExists(c19261966.filterchk,1,sg,tp,mg,sg,exg,mustg,fc,chkf)
	else
		res=sg:Includes(mustg) and c19261966.FCheckMixGoal(tp,sg,fc,true,true,chkf,c19261966.ffilter1,c19261966.ffilter2)
	end
	sg:RemoveCard(c)
	mg:Merge(rg)
	return res
...
function c19261966.fuscon(e,g,gc,chkf)
	...
	local mg=g:Filter(c19261966.ffilter,nil,c,SUMMON_TYPE_FUSION,tp)
	...
	if fc and fc:IsHasEffect(81788994) and fc:IsCanRemoveCounter(tp,0x16,3,REASON_EFFECT) then
		exg=Duel.GetMatchingGroup(c19261966.exfilter,tp,0,LOCATION_MZONE,nil,g,c,SUMMON_TYPE_FUSION,tp)
		mg:Merge(exg)
	end
	...
	mg:Merge(mustg)
	return mg:IsExists(c19261966.filterchk,1,nil,tp,mg,Group.CreateGroup(),exg,mustg,c,chkf)
end
function c19261966.filterchk2(c,tp,mg,sg,exg,mustg,fc,chkf)
	return not exg:IsContains(c) and c19261966.filterchk(c,tp,mg,sg,exg,mustg,fc,chkf)
end
function c19261966.filterchk3(c,tp,mg,sg,exg,mustg,fc,chkf)
	return exg:IsContains(c) and c19261966.filterchk(c,tp,mg,sg,exg,mustg,fc,chkf)
end
function c19261966.fusop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	local tp=e:GetHandlerPlayer()
	local exg=Group.CreateGroup()
	local mg=eg:Filter(c19261966.ffilter,nil,c,SUMMON_TYPE_FUSION,tp)
	...
	if fc and fc:IsHasEffect(81788994) and fc:IsCanRemoveCounter(tp,0x16,3,REASON_EFFECT) then
		local sg=Duel.GetMatchingGroup(c19261966.exfilter,tp,0,LOCATION_MZONE,nil,eg,c,SUMMON_TYPE_FUSION,tp)
		exg:Merge(sg)
		mg:Merge(sg)
	end
	if mustg:GetCount()>2 or (Auxiliary.FCheckExact and Auxiliary.FCheckExact~=2) or not mg:Includes(mustg) or mustg:IsExists(aux.NOT(Card.IsCanBeFusionMaterial),1,nil,c) then return false end
	...
	while sg:GetCount()<2 do
		local tg=mg:Filter(c19261966.filterchk2,sg,tp,mg,sg,exg,mustg,c,chkf)
		local tg2=mg:Filter(c19261966.filterchk3,sg,tp,mg,sg,exg,mustg,c,chkf)
