...
function c43644025.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		return res and Duel.GetLocationCount(tp,LOCATION_MZONE)>-1
			and Duel.CheckReleaseGroupCost(tp,c43644025.filter1,1,false,aux.ReleaseCheckMMZ,nil,e,tp) end
	e:SetLabel(0)
	local rg=Duel.SelectReleaseGroupCost(tp,c43644025.filter1,1,1,false,aux.ReleaseCheckMMZ,nil,e,tp)
