...
function c94977269.FCheckMix(c,mg,sg,fc,sub,sub2,fun1,fun2,...)
	if fun2 then
		sg:AddCard(c)
		local res=false
		if fun1(c,fc,SUMMON_TYPE_FUSION,tp) then
			res=mg:IsExists(c94977269.FCheckMix,1,sg,mg,sg,fc,sub,sub2,fun2,...)
		end
		sg:RemoveCard(c)
		return res
	else
...
function c94977269.FCheckMixGoal(tp,sg,fc,sub,sub2,chkf,...)
	local g=Group.CreateGroup()
	return sg:IsExists(c94977269.FCheckMix,1,nil,sg,g,fc,sub,sub2,...) and (chkf==PLAYER_NONE or Duel.GetLocationCountFromEx(tp,tp,sg,fc)>0)
...
function c94977269.filterchk(c,tp,mg,sg,exg,mustg,fc,chkf)
	...
	if sg:GetCount()<2 then
		...
		res=sg:Includes(mustg) and c94977269.FCheckMixGoal(tp,sg,fc,true,true,chkf,c94977269.ffilter1,c94977269.ffilter2)
	end
	sg:RemoveCard(c)
	mg:Merge(rg)
	return res
