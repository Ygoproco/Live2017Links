...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return g:GetCount()>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)+g:FilterCount(s.mzfilter,nil,tp)>0
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	ct1=ct1-1
	while ct1>0 and g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) do
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
