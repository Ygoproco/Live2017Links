...
function c86871614.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,86871615,0,TYPES_TOKEN,ec:GetBaseAttack(),ec:GetBaseDefense(),
...
function c86871614.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,86871615,0,TYPES_TOKEN,ec:GetBaseAttack(),ec:GetBaseDefense(),
			ec:GetOriginalLevel(),ec:GetOriginalRace(),ec:GetOriginalAttribute()) then return end
	ec:RegisterFlagEffect(86871614,RESET_EVENT+RESETS_STANDARD_EXC_GRAVE,0,0)
