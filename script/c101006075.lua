...
function c101006075.initial_effect(c)
	...
	e1:SetTarget(c101006075.target)
	e1:SetOperation(c101006075.activate)
	c:RegisterEffect(e1)
end
	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c101006075.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg3,mf)
			end
		end
		return res
	end
...
function c101006075.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c101006075.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf)
	end
