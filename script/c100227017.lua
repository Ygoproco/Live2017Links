...
function c100227017.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e:SetLabel(0)
		local mg=Duel.GetMatchingGroup(c100227017.cfilter,tp,LOCATION_MZONE,0,nil)
		local gg=not Duel.IsPlayerAffectedByEffect(tp,69832741) and Duel.GetMatchingGroup(c100227017.cfilter,tp,LOCATION_GRAVE,0,nil) 
			or Group.CreateGroup()
		local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
		return ((mg:GetCount()>0 and mg:FilterCount(c100227017.mzfilter,nil)+ft>0) or (gg:GetCount()>0 and ft>0)) 
			and Duel.IsExistingMatchingCard(c100227017.filter,tp,LOCATION_HAND,0,1,nil,e,tp)
	end
	local g
	if Duel.IsPlayerAffectedByEffect(tp,69832741) then
		g=Duel.GetMatchingGroup(c100227017.cfilter,tp,LOCATION_MZONE,0,nil)
	else
		g=Duel.GetMatchingGroup(c100227017.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
	end
...
function c100227017.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,c100227017.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp):GetFirst()
	if tc then
		if Duel.SpecialSummonStep(tc,0,tp,tp,true,false,POS_FACEUP) then
			...
			e3:SetValue(c100227017.efilter)
