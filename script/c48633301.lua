...
function c48633301.initial_effect(c)
	...
	e1:SetCondition(c48633301.spcon)
	c:RegisterEffect(e1)
	--search
...
function c48633301.spfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x27)
end
function c48633301.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c48633301.spfilter,tp,LOCATION_MZONE,0,1,nil)
end
...
function c48633301.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c48633301.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c48633301.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c48633301.filter,tp,LOCATION_DECK,0,1,1,nil)
	if #g>0 then
