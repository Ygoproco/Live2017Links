...
function c83107873.initial_effect(c)
	...
	e1:SetCost(c83107873.spcost)
	e1:SetTarget(c83107873.sptg)
	e1:SetOperation(c83107873.spop)
	c:RegisterEffect(e1)
	--to deck
	...
	e2:SetTarget(c83107873.tdtg)
	e2:SetOperation(c83107873.tdop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	e3:SetCode(EVENT_TO_GRAVE)
	e3:SetCondition(c83107873.tdcon)
	c:RegisterEffect(e3)
end
function c83107873.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c83107873.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c83107873.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c83107873.spfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
	if tc then
		Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c83107873.tdcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD)
end
function c83107873.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp)
		and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,LOCATION_HAND,0,1,e:GetHandler()) end
	Duel.SetTargetPlayer(tp)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_HAND)
end
function c83107873.tdop(e,tp,eg,ep,ev,re,r,rp)
