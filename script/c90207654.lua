...
function c90207654.spcheck(sg,e,tp,mg)
	return Duel.GetMZoneCount(tp,sg,tp,LOCATION_REASON_TOFIELD)>=#sg and Duel.IsExistingMatchingCard(c90207654.spfilter,tp,LOCATION_DECK,0,#sg,nil,e,tp)
end
function c90207654.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return aux.SelectUnselectGroup(g,e,tp,1,3,c90207654.spcheck,0) end
...
function c90207654.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local rg=aux.SelectUnselectGroup(g,e,tp,1,ct,c90207654.spcheck,1,tp,HINTMSG_REMOVE)
	if #rg>0 and Duel.Remove(rg,POS_FACEUP,REASON_EFFECT)~=0 then
		...
		local sg=Duel.SelectMatchingCard(tp,c90207654.spfilter,tp,LOCATION_DECK,0,ct,ct,nil,e,tp)
