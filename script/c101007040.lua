...
function c101007040.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,101007040+100,0xd9,0x4011,0,0,1,RACE_ZOMBIE,ATTRIBUTE_FIRE,POS_FACEUP) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0)
end
function c101007040.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,101007040+100,0xd9,0x4011,0,0,1,RACE_ZOMBIE,ATTRIBUTE_FIRE,POS_FACEUP)
		then return 
	end
	local token=Duel.CreateToken(tp,101007040+100)
