...
function c80280737.filter1(c,e,tp,ft)
	return c:IsType(TYPE_SYNCHRO) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) 
		and Duel.IsExistingMatchingCard(c80280737.filter2,tp,LOCATION_DECK,0,1,nil,c:GetCode(),e,tp)
end
...
function c80280737.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return ft>-1 and Duel.CheckReleaseGroupCost(tp,c80280737.filter1,1,false,nil,nil,e,tp,ft)
	end
	local rg=Duel.SelectReleaseGroupCost(tp,c80280737.filter1,1,1,false,nil,nil,e,tp,ft)
