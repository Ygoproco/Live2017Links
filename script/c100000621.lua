...
function c100000621.fop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	if gc then
		...
		while not tc or tc==gc do
			...
			tc=Group.SelectUnselect(mg:Filter(c100000621.filterchk,sg,tp,mg,sg,c),sg,p)
		end
		sg:AddCard(tc)
	else
		...
		::sec::
		Duel.Hint(HINT_SELECTMSG,p,HINTMSG_FMATERIAL)
		local sc2=Group.SelectUnselect(g2:Filter(c100000621.filterchk,sg,tp,mg,sg,c),sg,p,true,true)
