...
function c18236002.initial_effect(c)
	...
	e1:SetCondition(c18236002.reccon)
	e1:SetTarget(c18236002.rectg)
	e1:SetOperation(c18236002.recop)
	c:RegisterEffect(e1)
end
function c18236002.reccon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local rc=c:GetReasonCard()
	return c:IsLocation(LOCATION_GRAVE) and rc:IsSetCard(0x120) and r & REASON_FUSION+REASON_LINK ~=0
end
function c18236002.rectg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c18236002.recop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c18236002.spfilter,tp,LOCATION_DECK+LOCATION_HAND,0,nil,e,tp)
	if Duel.Damage(1-tp,500,REASON_EFFECT)~=0 and #g>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(18236002,1)) then
		Duel.BreakEffect()
