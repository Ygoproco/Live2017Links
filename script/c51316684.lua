...
function c51316684.spcon(e,c)
	...
	if c:IsHasEffect(34822850) then
		...
		if Duel.IsPlayerAffectedByEffect(tp,69832741) then
			return Duel.IsExistingMatchingCard(c51316684.spfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,2,c)
		else
...
function c51316684.spop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	if c:IsHasEffect(34822850) then
		...
		if Duel.IsPlayerAffectedByEffect(tp,69832741) then
			if ft>0 then
				g=Duel.SelectMatchingCard(tp,c51316684.spfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,2,2,c)
			else
				local sg=Duel.GetMatchingGroup(c51316684.spfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,c)
				local ct=-ft+1
				g=sg:FilterSelect(tp,Card.IsLocation,ct,ct,nil,LOCATION_MZONE)
				if ct<2 then
					sg:Sub(g)
...
function c51316684.rmfilter(c,e)
...
function c51316684.rmcost2(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c51316684.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,nil,e)
...
function c51316684.rmtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(1-tp) and c51316684.rmfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c51316684.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c51316684.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,1,0,0)
end
