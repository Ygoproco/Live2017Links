...
function c100334005.initial_effect(c)
	...
	e1:SetCondition(c100334005.rthcon)
	e1:SetCost(c100334005.rthcost)
	e1:SetTarget(c100334005.rthtg)
	e1:SetOperation(c100334005.rthop)
	c:RegisterEffect(e1)
	--recover
	...
	e2:SetCondition(c100334005.thcon)
	e2:SetCost(c100334005.thcost)
	e2:SetTarget(c100334005.thtg)
	e2:SetOperation(c100334005.thop)
	c:RegisterEffect(e2)
end
function c100334005.rthcon(e,tp,eg,ep,ev,re,r,rp)
...
function c100334005.rthcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c100334005.rthop(e,tp,eg,ep,ev,re,r,rp)
	local bc=e:GetLabelObject()
...
function c100334005.thcfilter(c,tp)
	return c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
		and Duel.IsExistingTarget(c100334005.thfilter,tp,LOCATION_GRAVE,0,1,c)
end
...
function c100334005.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100334005.thcfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c100334005.thcfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil,tp)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c100334005.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c100334005.thfilter(chkc) end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local sg=Duel.SelectTarget(tp,c100334005.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
