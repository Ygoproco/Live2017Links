...
function c53855409.ttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,53855410,0,TYPES_TOKEN,400,400,1,RACE_WARRIOR,ATTRIBUTE_DARK) end
...
function c53855409.top(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) or Duel.GetLocationCount(tp,LOCATION_MZONE)<2 
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,53855410,0,TYPES_TOKEN,400,400,1,RACE_WARRIOR,ATTRIBUTE_DARK) then return end
	for i=1,2 do
