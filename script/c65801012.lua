...
function c65801012.spfilter(c,e,tp,m,f,chkf)
...
function c65801012.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if not Duel.IsExistingMatchingCard(c65801012.exmfilter,tp,LOCATION_MZONE,0,1,nil) then
			local sg=Duel.GetMatchingGroup(c65801012.exfilter0,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
			if #sg>0 then
				mg1:Merge(sg)
				Auxiliary.FCheckAdditional=c65801012.fcheck
			end
		end
		local res=Duel.IsExistingMatchingCard(c65801012.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c65801012.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c65801012.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetFusionMaterial(tp):Filter(c65801012.filter,nil,e)
	local exmat=false
	if not Duel.IsExistingMatchingCard(c65801012.exmfilter,tp,LOCATION_MZONE,0,1,nil) then
		local sg=Duel.GetMatchingGroup(c65801012.exfilter1,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil,e)
		if #sg>0 then
			mg1:Merge(sg)
			exmat=true
		end
	...
	local sg1=Duel.GetMatchingGroup(c65801012.spfilter,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
	Auxiliary.FCheckAdditional=nil
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c65801012.spfilter,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
