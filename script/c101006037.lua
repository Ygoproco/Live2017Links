...
function c101006037.initial_effect(c)
	--fusion summon
	c:EnableReviveLimit()
	aux.AddFusionProcMixN(c,true,true,aux.FilterBoolFunction(Card.IsFusionSetCard,0x222),3)
	--spsummon condition
	...
	e2:SetCondition(c101006037.hspcon)
	e2:SetOperation(c101006037.hspop)
	c:RegisterEffect(e2)
	--destroy
	...
	e4:SetTarget(c101006037.desreptg)
	c:RegisterEffect(e4)
end
function c101006037.spfilter1(c)
	return c:IsRace(RACE_THUNDER) and c:IsAbleToRemoveAsCost() and (c:IsLocation(LOCATION_HAND) or aux.SpElimFilter(c,true))
end
function c101006037.spfilter2(c)
	return c:IsFaceup() and c:IsRace(RACE_THUNDER) and c:IsType(TYPE_FUSION) and not c:IsCode(101006037) and c:IsAbleToRemoveAsCost()
end
function c101006037.rescon(sg,e,tp,mg)
	return Duel.GetLocationCountFromEx(tp,tp,sg)>0 and sg:IsExists(c101006037.spfilter1,1,nil) and sg:IsExists(c101006037.spfilter2,1,nil)
end
function c101006037.hspcon(e,c)
...
function c101006037.hspop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g=g1:Clone()
	g:Merge(g2)
	local sg=aux.SelectUnselectGroup(g,e,tp,2,2,c101006037.rescon,1,tp,HINTMSG_REMOVE)
...
function c101006037.repfilter(c)
