...
function s.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>1 and
		Duel.IsPlayerCanSpecialSummonMonster(tp,id+100,0,0x4011,at:GetAttack(),0,2,RACE_AQUA,ATTRIBUTE_WATER) 
	end
...
function s.tkop(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if c:GetSequence()==seq and Duel.IsPlayerCanSpecialSummonMonster(tp,id+100,0,0x4011,at:GetAttack(),0,2,RACE_AQUA,ATTRIBUTE_WATER) then
		Duel.BreakEffect()
