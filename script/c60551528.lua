...
function c60551528.spcon(e,c)
	...
	if c:IsHasEffect(34822850) then
		...
		if Duel.IsPlayerAffectedByEffect(tp,69832741) then
			return Duel.IsExistingMatchingCard(c60551528.spfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,1,c)
		else
...
function c60551528.spop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	if c:IsHasEffect(34822850) then
		...
		if Duel.IsPlayerAffectedByEffect(tp,69832741) then
			if ft>0 then
				g=Duel.SelectMatchingCard(tp,c60551528.spfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,1,1,c)
			else
				g=Duel.SelectMatchingCard(tp,c60551528.spfilter,tp,LOCATION_MZONE,0,1,1,c)
			end
		else
...
function c60551528.rmfilter(c,e)
...
function c60551528.rmcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c60551528.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,nil,e)
...
function c60551528.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_GRAVE) and chkc:IsControler(1-tp) and c60551528.rmfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c60551528.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c60551528.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,1,nil)
