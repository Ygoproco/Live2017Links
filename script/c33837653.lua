...
function c33837653.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(33837653,1))
	...
	e4:SetDescription(aux.Stringid(33837653,1))
...
function c33837653.ntcon(e,c,minc)
	...
	return minc==0 and c:GetLevel()>4 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and ((Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)>0 and Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==0) or Duel.IsExistingMatchingCard(c33837653.cfilter,tp,LOCATION_MZONE,0,1,nil))
end
...
function c33837653.discon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tg and tg:IsExists(c33837653.filter2,1,nil,tp) and Duel.IsChainNegatable(ev)
end
