...
function s.synfilter(c,mg)
	return c:IsSetCard(0x231) and c:IsType(TYPE_SYNCHRO) and (not mg or c:IsSynchroSummonable(nil,mg,#mg,#mg))
end
...
function s.target(fil,con)
	return function(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
		...
		if chkc then return chkc:IsControler(tp) and c:IsLocation(LOCATION_GRAVE) and c:IsSetCard(0x231) and chkc:IsCanBeSpecialSummoned(e,0,tp,false,false) and cancelcon(Group.FromCards(chkc)) end
