...
function c47660516.filter1(c,e,tp)
	...
	return pg:GetCount()<=1 and c:IsFaceup() and (rk>0 or c:IsStatus(STATUS_NO_LEVEL)) and Duel.GetLocationCountFromEx(tp,tp,c)>0 
		and Duel.IsExistingMatchingCard(c47660516.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,rk+1,c:GetRace(),pg)
end
function c47660516.filter2(c,e,tp,mc,rk,rc,pg)
...
function c47660516.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c47660516.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank()+1,tc:GetRace(),pg)
	local sc=g:GetFirst()
	if sc then
