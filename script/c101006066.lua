...
function c101006066.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(101006066,0))
	...
	e2:SetDescription(aux.Stringid(101006066,1))
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1,101006066)
	e2:SetCost(c101006066.dcost)
...
function c101006066.dcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local ec=c:GetEquipTarget()
...
function c101006066.filter(c,g,tc)
...
function c101006066.dtarget(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local ec=c:GetEquipTarget()
	if not ec then return end
	local g=Duel.GetMatchingGroup(c101006066.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,c,ec:GetColumnGroup(),ec)
...
function c101006066.doperation(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg=Duel.SelectMatchingCard(tp,c101006066.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tc:GetColumnGroup(),tc)
	if #dg>0 then
