...
function c56641453.cfilter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x1e) and Duel.IsExistingMatchingCard(c56641453.spfilter,tp,0x13,0,1,nil,c,e,tp)
end
...
function c56641453.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e:SetLabel(0)
		return res and Duel.CheckReleaseGroupCost(tp,c56641453.cfilter,1,false,aux.ReleaseCheckMMZ,nil,e,tp)
	end
	e:SetLabel(0)
...
function c56641453.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c56641453.spfilter),tp,0x13,0,1,1,nil,tc,e,tp)
