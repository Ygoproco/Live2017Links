...
function s.thcfilter(c,e,tp)
	...
	return c:IsSetCard(0x236) and c:IsSummonPlayer(tp) and c:IsLocation(LOCATION_MZONE) and c:IsCanBeEffectTarget(e)		
		and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil,code)
end
function s.thfilter(c,code)
	return c:IsSetCard(0x236) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not c:IsCode(code)
end
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x236) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
