...
function s.filter1(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_LINK) and c:IsSetCard(0x119)
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function s.filter2(c,e,tp,mc)
	return c:IsSetCard(0x119) and mc:IsCanBeLinkMaterial(c,tp) and c:IsType(TYPE_LINK)  and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0 
