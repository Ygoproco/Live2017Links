...
c1142880.listed_names={CARD_CYBER_DRAGON }
function c1142880.cfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsDiscardable()
end
function c1142880.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c1142880.spfilter(c,e,tp)
...
function c1142880.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c1142880.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c1142880.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c1142880.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c1142880.splimit(e,c,sump,sumtype,sumpos,targetp,se)
