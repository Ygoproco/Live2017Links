...
function s.costfilter(c)
	return c:IsRace(RACE_WINDBEAST) and c:IsType(TYPE_MONSTER) and c:IsDiscardable()
end
...
function s.lvop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.ShuffleHand(tp)
	local hg=Duel.GetMatchingGroup(s.afilter,tp,LOCATION_HAND,0,nil,g:GetFirst():GetCode()):ForEach(function(tc) tc:UpdateLevel(-1,RESET_EVENT+0xfe0000+RESET_PHASE+PHASE_END,e:GetHandler()) end)
end
