...
function c100410008.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct>=1 then
		...
		c:RegisterEffect(e1)
		if ct==1 then
		c:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(100410008,0)) end
	end
	if ct>=2 then
		local e2=Effect.CreateEffect(c)
		...
		c:RegisterEffect(e3)
		if ct==2 then
		c:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(100410008,1)) end
	end
	if ct==3 then
